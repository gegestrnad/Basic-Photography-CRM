...
import { LayoutDashboard, Briefcase, Wallet, CheckSquare, Calculator, Users, Settings, ShieldCheck } from 'lucide-react';
import type { ViewKey } from '@/lib/types';

export interface NavItem {
  key: ViewKey;
  labelKey: 'nav_dashboard' | 'nav_jobs' | 'nav_payments' | 'nav_tasks' | 'nav_wages' | 'nav_clients' | 'nav_settings' | 'nav_admin';
  icon: React.ComponentType<{ className?: string }>;
  /** Shown in the mobile bottom nav (5-slot grid). When false, only appears in desktop sidebar. */
  mobile: boolean;
  /** Restrict visibility to specific roles. When undefined, all authenticated users see it. */
  roles?: ('admin' | 'user')[];
}
...
export const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'admin',     labelKey: 'nav_admin',     icon: ShieldCheck,     mobile: false, roles: ['admin'] },
];
